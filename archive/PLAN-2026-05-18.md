...
- Varz / NATS HTTP monitoring 不对外开放；Web 端不实现相关读取和展示。
...
- 实现可组合的业务能力。
- 例如 subject 归一化、服务器 profile 管理、request payload 处理、最新值模型转换。
- L3 不拥有跨功能编排权。
...
状态：已调整（2026-05-18，移除 Varz / monitoring 展示）。

目标：

- 在 Web 中显示服务器连接状态、主动探测结果和未连接原因。
- 不读取或展示 Varz / NATS HTTP monitoring 信息。

范围内：

- 当前 Profile 的全部 servers。
- 当前实际连接服务器的 `connected` 状态。
- 其他服务器的主动 WSS 探测结果和未连接原因。

范围外：

- 不实现完整 NATS 管理后台。
- 不实现 Varz、Healthz、Connz、Subsz 等 HTTP monitoring 展示。

完成定义：

- Node 模块显示当前 Profile 的全部 servers；当前实际连接服务器显示 `connected`，其他服务器显示主动 WSS 探测结果和未连接原因。
- 页面不出现 Varz 入口、弹窗或 monitoring 请求。

必要验证：

- 类型检查通过。
- 浏览器端服务器节点不显示 Varz 入口。

风险：

- 只保留 WSS 探测能得到的信息；底层 NATS server id、version、uptime 等字段不再展示。
...
- 点击复制 URL 后，URL 中包含 servers、watchSubject、requestSubject、requestPayload、timeout 等非敏感字段。
...
4. NATS server info 是否必须显示更多底层连接信息。当前计划只保证连接状态，不依赖 Varz / HTTP monitoring。
